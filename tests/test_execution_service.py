...
class MinNotionalMarketDataService:
    def get_symbol_rules(self, pair_symbol: str) -> SymbolRules:
        return SymbolRules(
            pair_symbol=pair_symbol,
            price_scale=2,
...
def test_dry_run_marks_would_reject_min_notional_without_submit(tmp_path) -> None:
    store = StateStore(db_path=str(tmp_path / "state.db"))
    exchange = RecordingExchange()
    service = ExecutionService(
        exchange=exchange,
    ...
    assert placed == 0
    assert exchange.placed == []
    assert service.last_execute_summary["would_reject_min_notional"] == 1
    assert service.last_execute_summary["would_submit_orders"] == 0


def test_live_submit_uses_quantized_notional_path(tmp_path) -> None:
    store = StateStore(db_path=str(tmp_path / "state.db"))
    exchange = RecordingExchange()
    service = ExecutionService(
        exchange=exchange,
    ...
    assert placed == 1
    assert len(exchange.placed) == 1
    symbol, side, price, quantity, _ = exchange.placed[0]
    assert symbol == "BTCTRY"
